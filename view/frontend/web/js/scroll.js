        ...
        initialize: function () {
            ...
            if (typeof initialUrl === 'undefined') {
                ...
                $(window).scroll(function () {
                 if ($(window).scrollTop() >= (self.options.scrollStart / 100) *
                     ($(document).height() - $(window).height())) {
                        let url = $('.next').attr('href');
                        if (typeof url != 'undefined' && !(self.options.lists.includes(url))) {
                            self.autoLoadProducts(url);
                            self.options.lists.push(url);
                        }
                    }
                ...
                let link = $(options).find('.next').attr('href');
                 ...
                 $.each(productArr,function (index, value) {
                    let length = $(value.firstElementChild.attributes.length);
                     if (length !== 1) {
                         childProduct = true;
                         $(self.options.productItemList).append(value).trigger('contentUpdated');
